...
<div class="row page-content-title">
  <div class="col-sm-8">
    <h1>Inspection and Acceptance Report (IAR) / Inbox</h1>
  </div>
  <div class="col-sm-4 col-button">
    @if(request()->route()->getName() == 'iar.preparation_inbox')
      <a class="btn btn-primary pull-right" href="{{ url('iar/preparation') }}" role="button">New</a>
    @endif
  </div>
</div>
  ...
  <div class="card-body">
    <div class="table-responsive">
      <table class="table table-bordered table-striped" width="100%" id="sai-inbox" cellspacing="0">
        ...
        <tbody>
          @foreach($iar as $header)
            <tr data-id="{{ $header['IARHeaderID'] }}" data-controlno="{{ $header['IARControlNo'] }}">
              <td>{{ $header['IARControlNo'] }}</td>
...
<script type="text/javascript">
  (function() {
    ...
    $('tbody tr').on('click', 'td:not(:first-child)', function() {
      window.location = base_url + 'iar/preparation/IARControlNo/' + $(this).parent().data('controlno');
    });
