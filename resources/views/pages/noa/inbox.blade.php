...
<div class="row page-content-title">
  <div class="col-sm-8">
    <h1>Notice of Award (NOA) / Inbox</h1>
  </div>
  <div class="col-sm-4 col-button">
    @if(request()->route()->getName() == 'noa.preparation_inbox')
      <a class="btn btn-primary pull-right" href="{{ url('noa/preparation') }}" role="button">New</a>
    @endif
  </div>
</div>
<div class="card mb-3">
      ...
      <table class="table table-bordered table-striped" width="100%" id="sai-inbox" cellspacing="0">
        ...
        <tbody>
          @foreach($row as $header)
            <tr data-id="{{ $header['NOAHeaderID'] }}" data-controlno="{{ $header['NOAControlNo'] }}">
              <td>{{ $header['NOAControlNo'] }}</td>
...
<script type="text/javascript">
  (function() {
    ...
    $('tbody tr').on('click', 'td:not(:first-child)', function() {
      window.location = base_url + 'noa/preparation/NOAControlNo/' + $(this).parent().data('controlno');
    });
